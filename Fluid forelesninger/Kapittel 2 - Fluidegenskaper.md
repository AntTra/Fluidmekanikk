## 2.1 Kontinuumshypotesen
Vi betrakter massen som kontinuerlig fordelt, selv i små volum dxdydz

## Tetthet/density
Beskrevet med $\rho$. 

#### Relativ tetthet: SG 
$SG = \frac{\rho_fluid}{\rho_vann}$. Vi sammenlikner tettheten til et fluid med vann.
$\rho$ \[m/volum\]
#### Spesifikk vekt: 
Spesifikk vekt = $\gamma = \rho*g [N/m^3]$

#### Tilstandslikning/State equation:
Kobling mellom termodynamisk variable tetthet $\rho$, trykk $p$, og temperatur $T$

-> Idéell gasslov: $p = \rho R T$, der p er absolutt trykk, R er den universelle gasskonstanten/Massekonstant for det vi ser på. For luft så er R = 287. Temperatur er i kelvin

Tilstandslikning for væske, $\rho \simeq konstant$ 

## 2.3 Damptrykk $P_v$
Vapor pressure. Tegn trekant for fri overflate. 

$P_v =$ trykk for væske som er i likevekt ved en gitt temperatur $=>$ væsken koker. Vann ved 100 grader

## Eksempel: Koke egg på Elbrush 5000 m.o.h.
A11: $p \sim \frac{1}{2}atm = 54kPa$
A3: $T_{sat} \sim 80 C$

#### Kavitasjon (=koker på grunn av strømning):
Trykket blir så lavt p.g.a. strømningsforhold at trykket begynner å nærme seg damptrykket. Vil ikke endre viskositeten. Kavitasjon kan få deg til å blø lol

## 2.6 Viskositet 
Newtons friksjonslov
Vinkel = sirkelbue/radius
> For små vinkler så er vinkelen ca det samme som tan av vinkelen og sinus av vinkelen

Vinkeldeformasjonshastighet $\dot{\beta}=du/dy$ 
$\beta = tan(\beta)=du*dt/dy$
![[Pasted image 20260119140643.png]] Noter differansene og hvor de er null og ikke

#### Eksempel - Viskositetsmåler

## 2.7 Overflatespenning
$\sigma_s=N/m$ 
![[Pasted image 20260119185853.png]]
![[Pasted image 20260119190329.png]]
![[Pasted image 20260119190519.png]]

#### Kontaktvinkel
Sier om en væske vil fukte overflaten eller ikke ved bruk av vinkelen for kontaktkanven av væsken og overflaten. $\phi$
![[Pasted image 20260119190756.png]]

Kapillæreffekt illustrasjon:
![[Pasted image 20260119191109.png]]

![[Pasted image 20260119193234.png]]
![[Pasted image 20260119193248.png]]